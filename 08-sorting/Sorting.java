...
public class Sorting {
    ...
    // Non-Magic values for testing purposes
    private static final int startSize = 2;
    private static final int endSize = 100000;
    ...
    public static final int nanoToMicro = 1000;

    /**
     * This method provides testing for merge sort and insertion sort.
     * In order to get a better idea of the performance of these two sorting
     * algorithms, we can time how long it takes for them to finish executing
     * with an increasing number of elements. To accomplish this, we create
     * a new array of *startSize* elements. Then, loop while arraySize < endSize,
     * increaing by *scaleFactor* after each subsequent run. In order to account
    ...
    public static void experiment() throws IOException {
        ...
        writer.append("Run, Insertion Sort, Merge Sort\n");
        int cnt = 0;
        while (cnt < numRuns) {
            int arraySize = startSize;
            int count = 1;
            while (arraySize < endSize) {

                int[] test = randomArray(arraySize, minRandomValue, maxRandomValue);
                String output = "";
                long startTime = System.nanoTime();
                insertionSort(test);
                double howLongInsertion = (double) (System.nanoTime() - startTime) / nanoToMicro;
                output += "Insertion Sort took: " + howLongInsertion + " microseconds on run: " + count + " \n";
                test = randomArray(arraySize, minRandomValue, maxRandomValue);
                startTime = System.nanoTime();
                mergeSort(test);
                double howLongMerge = (double) (System.nanoTime() - startTime) / nanoToMicro;
                output += "Merge Sort took: " + howLongMerge + " microseconds on run: " + count;
                System.out.println(output);
                writer.append(count + ", " + howLongInsertion + ", " + howLongMerge + "\n");
